...
Matrix_OF* Matrix_OF::multiply(const Matrix_OF* const m) const
{
    ...
    if(this->getInfo().rowCount == m->getInfo().colCount)
    ...
    {
        for (size_t j = 0; j < m->getInfo().size; ++j) 
        {
            if (this->matrix[i]->getCol() == m->matrix[j]->getRow()) 
            {
//                auto point = std::find_if(stat->begin() , stat->end() , [&](const cellInfo* const c){
//                return (c->i == this->matrix[i]->getRow() && c->j == m->matrix[j]->getCol());
//                });
                size_t k = 0;
                for (; k < stat->size(); k++) {
                    if(stat[k]->i == this->matrix[i]->getRow() && stat[k]->j == m->matrix[j]->getCol())
                     {
                        stat[k]->add(i , j);
                        break;
                     }
                ...
                if(k==stat->size())
                {
                    cellInfo* c = new cellInfo(this->matrix[i]->getRow() , m->matrix[j]->getCol());
                    c->add(i,j);
                    stat->push_back(c);
                }
            }   
